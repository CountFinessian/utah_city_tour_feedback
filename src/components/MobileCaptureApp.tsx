    ...
    <div className="h-full flex flex-col justify-between overflow-hidden gap-2 flex-1 min-h-0">
      ...
      <header className="flex items-center justify-between py-1 shrink-0">
        <div>
          ...
          <h1 className="text-lg sm:text-xl font-black text-white tracking-tight leading-tight">Guided tour debrief</h1>
        </div>
        <div className="flex items-center gap-2">
          {currentUser?.role === "leader" && (
            <Link
              href="/command"
              className="text-xs font-bold px-2.5 py-1.5 rounded-lg bg-white/[0.06] border border-white/10 text-slate-200 hover:bg-white/10 transition-colors"
            >
              Command
            </Link>
          )}
          {currentUser && (
            <button
              ...
              className="text-xs sm:text-sm px-3 py-1.5 rounded-lg bg-emerald-500/15 border border-emerald-500/30 text-emerald-200 font-bold flex items-center gap-1.5 h-8 hover:bg-emerald-500/25 transition-colors cursor-pointer"
      ...
      <div className="flex items-center gap-2 py-1.5 px-3 rounded-xl bg-white/[0.03] border border-white/5 text-xs sm:text-sm text-slate-200 font-semibold shrink-0 mb-1">
          ...
          <div className="w-full max-w-md rounded-2xl border border-[#26354c] bg-[#101827] p-5 sm:p-6 shadow-2xl space-y-4 max-h-[90vh] overflow-y-auto">
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="text-base sm:text-lg font-bold text-white">Account & AI Privacy</h3>
                  <p className="text-xs sm:text-sm text-slate-300 font-medium">{currentUser?.email || "Host Account"}</p>
                </div>
              </div>
              <button
                type="button"
            ...
            <div className="p-4 rounded-xl bg-white/[0.02] border border-white/10 space-y-2.5">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2 text-[#43d9c7] font-bold text-sm sm:text-base">
            ...
            <div className="p-3.5 rounded-xl bg-white/[0.02] border border-white/10 flex items-center justify-between text-xs sm:text-sm">
            ...
            <div className="pt-2 border-t border-white/10 space-y-3">
              {!showDeleteConfirm ? (
                <div className="p-4 rounded-xl bg-red-500/[0.04] border border-red-500/20 space-y-3">
                  <div>
                    <p className="text-sm sm:text-base font-bold text-red-200">Delete Account</p>
                    <p className="text-xs sm:text-sm text-slate-300 mt-0.5 leading-relaxed font-medium">
    ...
    <div className="h-full flex flex-col justify-between overflow-hidden gap-3 py-1 flex-1 min-h-0">
          ...
          <div className="flex items-center gap-1.5">
            ...
            {transcript.length > 0 && (
              <button
                ...
                className="text-xs sm:text-sm text-slate-200 font-bold hover:text-white px-2.5 py-1 rounded-md hover:bg-white/10 transition cursor-pointer ml-1"
              >
                Clear
              </button>
            )}
          ...
          <div className="w-full max-w-md rounded-2xl border border-[#26354c] bg-[#101827] p-5 shadow-2xl space-y-3.5 animate-in fade-in">
            <div className="flex items-center justify-between border-b border-white/10 pb-2.5">
              <div className="flex items-center gap-2.5">
                ...
                <span className="text-base sm:text-lg font-bold text-white">Prospect Tracking</span>
              </div>
              <button
                type="button"
                onClick={onContextOpen}
            ...
            <label className="mobile-field">
              <span className="text-xs sm:text-sm font-bold uppercase tracking-wider text-slate-200">Tour Host</span>
              <input
                value={hostName}
                readOnly
                disabled
            ...
            <button
              type="button"
              onClick={onContextOpen}
              className="w-full mt-2 py-3 rounded-xl bg-[#36cdbd] text-[#070b12] font-black text-sm sm:text-base hover:bg-[#43d9c7] transition-colors cursor-pointer"
            >
              Done
            </button>
          </div>
      ...
      <div className="shrink-0 pt-3 pb-1 flex items-center justify-between gap-3 border-t border-white/10">
        <div className="min-w-0">
          ...
          <p className="text-xs sm:text-sm text-slate-300 font-medium truncate">Zero follow-up forms.</p>
        </div>
        <button
          type="button"
...
function MobileField({
  ...
  return (
    <label className="mobile-field">
      <span className="text-xs sm:text-sm font-bold uppercase tracking-wider text-slate-200">{label}</span>
      <input
        value={value}
      ...
      <div className="w-full max-w-md rounded-2xl border border-[#26354c] bg-[#0c121e] p-5 sm:p-6 shadow-2xl space-y-4 max-h-[92vh] overflow-y-auto">
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-lg sm:text-xl font-black text-white">AI Voice & Analysis Consent</h3>
              <p className="text-xs sm:text-sm text-slate-300 font-medium">Required before submitting or recording debriefs</p>
            </div>
          </div>
          <button
            type="button"
        ...
        <div className="space-y-3">
          <div className="p-3.5 rounded-xl bg-white/[0.03] border border-white/10 space-y-1">
            <div className="flex items-center gap-2 font-bold text-white text-sm sm:text-base">
              <Mic className="h-4 w-4 text-[#43d9c7]" />
        ...
        <div className="p-3 rounded-xl bg-white/[0.02] border border-white/5 flex items-center justify-between text-xs sm:text-sm">
          <span className="text-slate-300 font-medium">Policies & Help</span>
          <div className="flex items-center gap-3">
            <button
              type="button"
        ...
        <div className="pt-2 flex items-center justify-end gap-2.5">
          <button
            type="button"
            onClick={onDecline}
            className="px-4.5 py-3 rounded-xl border border-white/10 text-sm font-bold text-slate-200 hover:bg-white/5 transition-colors cursor-pointer"
          >
            Not Now
          </button>
          <button
            type="button"
            onClick={onAccept}
            className="px-5.5 py-3 rounded-xl bg-emerald-500 hover:bg-emerald-400 text-[#070b12] font-black text-sm sm:text-base shadow-lg shadow-emerald-500/20 transition-all cursor-pointer flex items-center gap-2"
          >
