...
export async function sendOpsAlertEmail({
  ...
  if (!apiKey) {
    console.warn("[mailer] RESEND_API_KEY not configured. Ops alert would go to:", to, "\n", body);
    return { success: false, error: "RESEND_API_KEY is not configured." };
  }

  const html = `
<!DOCTYPE html>
<html><body style="font-family:sans-serif;color:#111;line-height:1.5;padding:24px;">
  <p style="font-weight:700;margin:0 0 16px;">Utah City Ops Alert</p>
  <pre style="white-space:pre-wrap;font-size:13px;background:#f9fafb;padding:16px;border-radius:8px;border:1px solid #e5e7eb;">${body
    .replace(/&/g, "&amp;")
    .replace(/</g, "&lt;")
    .replace(/>/g, "&gt;")}</pre>
</body></html>`.trim();
  ...
  try {
    ...
    const data = await res.json();
    if (!res.ok) {
      console.error("[mailer ops alert error]", data);
      return { success: false, error: data.message || "Failed to send ops alert" };
    }
    return { success: true, id: data.id };
  } catch (err: any) {
    console.error("[mailer ops alert exception]", err);
    return { success: false, error: err.message || "Network error sending ops alert" };
  }
}
