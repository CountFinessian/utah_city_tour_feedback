  ...
  return (
    <div className="command-page space-y-5">
      ...
      <section className="space-y-5">
        ...
        <div id="leadership-action-items" className="space-y-3 scroll-mt-24">
          <div className="flex items-center justify-between gap-3">
            <div>
              <p className="command-label">Leadership Action Items</p>
              <h2 className="mt-1 text-lg font-semibold text-command-ink">Where to improve next</h2>
            </div>
            <span className="text-xs text-command-muted">Ontology actions · updated on each submit</span>
          </div>
          {view.recommendationRows.length === 0 ? (
            <div className="command-panel text-sm text-command-muted">No action item has enough evidence yet.</div>
          ) : (
            view.recommendationRows.map((action) => (
              <CommandActionTarget
                key={action.id}
                id={actionAnchorId(action.themeId || action.title)}
              >
                <RecommendationCard
                  title={action.title}
